...
ICM_20948_Status_e ICM_20948_write_I2C_bus(uint8_t reg, uint8_t *data, uint32_t len, void *user);
ICM_20948_Status_e ICM_20948_read_I2C_bus(uint8_t reg, uint8_t *buff, uint32_t len, void *user);
...
//    debugPrint(F("ICM_20948::startupDMP: DMP is not available. Please check that you have uncommented line 29 (#define ICM_20948_USE_DMP) in ICM_20948_C.h..."));
  ...
  // ICM_20948_Clock_Auto selects the best available clock source �C PLL if ready, else use the Internal oscillator
...
ICM_20948_Status_e ICM_20948_I2C::begin(uint8_t i2c_bus, uint8_t addr)
{
  // Associate
//  _ad0 = ad0pin;
//  _i2c = &wirePort;
...
ICM_20948_Status_e ICM_20948_write_I2C_bus(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
	...
	uint8_t bus = ((ICM_20948_I2C *)user)->_i2c_bus;
	uint8_t addr = ((ICM_20948_I2C *)user)->_addr;
...
void I2C_write(uint8_t addr, uint8_t reg, uint32_t len, uint8_t *data){
	HAL_I2C_Mem_Write(&hi2c1, addr<<1, reg, I2C_MEMADD_SIZE_8BIT, data, len, 1000);
};
void I2C_read(uint8_t addr, uint8_t reg, uint32_t len, uint8_t *buff){
	HAL_I2C_Mem_Read(&hi2c1, addr<<1, reg, I2C_MEMADD_SIZE_8BIT, buff, len, 1000);
};
